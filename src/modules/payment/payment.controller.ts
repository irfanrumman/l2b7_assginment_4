...
const getMyPaymentHistory = catchAsync(async (req: Request, res: Response) => {
  ...
  const result = await paymentService.getMyPaymentHistoryFromDB(tenantId, query);
...
const getPaymentById = catchAsync(async (req: Request, res: Response, next: NextFunction) => {
  ...
  const result = await paymentService.getPaymentByIdFromDB(paymentId, userId, role);
